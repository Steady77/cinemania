...
function kinobd_key(event) {
	...
	var key = '';
	'Enter' === event.key ||
		(13 === event.keyCode ? (key = 'fullscreen') : 'Left' === event.key) ||
		'ArrowLeft' === event.key ||
		(37 === event.keyCode ? (key = 'prev') : 'Right' === event.key) ||
		'ArrowRight' === event.key ||
		(39 === event.keyCode ? (key = 'next') : 'Up' === event.key) ||
		'ArrowUp' === event.key ||
		(38 === event.keyCode ? (key = 'up') : 'Down' === event.key) ||
		'ArrowDown' === event.key ||
		(40 === event.keyCode ? (key = 'down') : '0' === event.key) ||
		(48 === event.keyCode ? (key = '0') : '1' === event.key) ||
		(49 === event.keyCode ? (key = '1') : '2' === event.key) ||
		(50 === event.keyCode ? (key = '2') : '3' === event.key) ||
		(51 === event.keyCode ? (key = '3') : '4' === event.key) ||
		(52 === event.keyCode ? (key = '4') : '5' === event.key) ||
		(53 === event.keyCode ? (key = '5') : '6' === event.key) ||
		(54 === event.keyCode ? (key = '6') : '7' === event.key) ||
		(55 === event.keyCode ? (key = '7') : '8' === event.key) ||
		((56 === event.keyCode ? (key = '8') : '9' !== event.key) &&
			57 !== event.keyCode) ||
		(key = '9');
...
function kbp(self) {
	...
	var sel =
		self && self.getAttribute('data-kbd')
	...
	options.button_limit =
		options.button_limit && parseInt(options.button_limit) < 5
	...
	for (var da in data_kbd) {
		if (data_kbd.hasOwnProperty(da) && data_kbd[da]) {
			var kinobd_da = document.querySelector(
				'#' + data_kbd[da].getAttribute('data-kbd'),
			);
			if (kinobd_da) {
	...
	if (
		kinobd.parentNode &&
		kinobd.parentNode.tagName &&
		kinobd.parentNode.tagName.toLowerCase() === 'body'
	) {
		h = Math.max(
		...
		parseInt(kinobd.offsetHeight) &&
		parseInt(kinobd.offsetHeight) < 370
	) {
		if (
			kinobd.parentNode &&
			parseInt(kinobd.parentNode.offsetHeight) &&
			parseInt(kinobd.parentNode.offsetHeight) >= 370
		) {
		...
		parseInt(kinobd.offsetHeight) &&
		w / 3 < parseInt(kinobd.offsetHeight)
	) {
		h = parseInt(kinobd.offsetHeight);
	} else if (
		kinobd.parentNode &&
		parseInt(kinobd.parentNode.offsetHeight) &&
		w / 3 < parseInt(kinobd.parentNode.offsetHeight)
	) {
		h = parseInt(kinobd.parentNode.offsetHeight);
	} else {
		h = w / 2;
	...
	kb_get(options_url, p, function (players) {
		try {
			var first = true;
				...
				if (p.length) {
					...
					for (var ps = 0; ps < keys.length; ps++) {
						for (var pp = 0; pp < kb_res.length; pp++) {
							if (
								keys[ps].toLowerCase().indexOf(kb_res[pp].toLowerCase()) + 1
							) {
			...
			for (var i = 0, len = keys.length; i < len; i++) {
				...
				if (
					...
					if (
						j &&
						!(j % options.button_limit) &&
						players[keys[i + 1].toLowerCase().trim()] &&
						players[keys[i - 1].toLowerCase().trim()]
					) {
...
function kb_player(
	...
	var kinobdButtons = buttons
		? buttons
		: document.querySelector('#kinobd-buttons');
	...
	if (kinobdButtons) {
		...
		setTimeout(function () {
			var btn = setInterval(function () {
				if (
					parseInt((kinobdButtons.style && kinobdButtons.style.right) || '0') >
					-parseInt(kinobdButtons.offsetWidth) + 30 * size
				) {
					kinobdButtons.style.right =
						parseInt(kinobdButtons.style.right) - 1 + 'px';
				} else {
					clearInterval(btn);
				}
			}, 5);
...
function kb_ping(provider, loadtime = null) {
	...
	var kinopoisk = player.dataset.kinopoisk
		? encodeURIComponent(player.dataset.kinopoisk)
		: '';
